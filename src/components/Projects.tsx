...
const selectedProjects = projects.filter((project) => project.isFeatured).slice(0, 4);
  ...
  return (
    <article className={`group grid overflow-hidden rounded-xl border border-slate-200 bg-white lg:grid-cols-[1.08fr_0.92fr] ${isPaused ? 'opacity-80' : ''}`}>
      <div className={`relative min-h-64 overflow-hidden bg-slate-100 ${index % 2 === 1 ? 'lg:order-2' : ''}`}>
        {project.image ? (
          <img src={project.image} alt={project.title} className={`h-full min-h-64 w-full transition duration-700 group-hover:scale-[1.025] ${project.imageStyle || 'object-cover'}`} />
        ) : (
  ...
  return (
    <section id="projects" className="bg-white py-16 md:py-24 lg:py-28">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 lg:px-10">
        ...
        <div className="mt-12 space-y-6">{selectedProjects.map((project, index) => <SelectedProject key={project.title} project={project} index={index} />)}</div>
